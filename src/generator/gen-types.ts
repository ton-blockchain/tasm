  ...
  return [
    t.objectProperty(
      ...
      t.arrowFunctionExpression(
        ...
        t.blockStatement([
          t.expressionStatement(
            t.callExpression(t.memberExpression(t.identifier("s"), t.identifier("skip")), [
    ...
    t.objectProperty(
      ...
      t.arrowFunctionExpression(
        ...
        t.blockStatement([
          t.expressionStatement(
            t.callExpression(
              t.memberExpression(t.identifier("b"), t.identifier("storeInstructionPrefix")),
              [
                instruction.kind === "fixed-range" || instruction.kind === "ext-range"
                  ? t.binaryExpression(
                      ">>",
                      t.numericLiteral(instruction.prefix),
                      t.numericLiteral(instruction.skipLen - instruction.checkLen),
                    )
                  : t.numericLiteral(instruction.prefix),
                t.numericLiteral(instruction.checkLen),
                t.identifier("val"),
...
const generateOpcode = (name: string, instruction: $.Opcode): t.Statement[] => {
  ...
  const constructor = t.exportNamedDeclaration(
    t.variableDeclaration("const", [
      t.variableDeclarator(nameIdent, t.objectExpression(generateOpcodeFields(name, instruction))),
    ]),
  )
...
const generateArg = (arg: $.arg): t.Expression => {
  switch (arg.$) {
    ...
    case "exoticCell":
      return t.callExpression(t.memberExpression(UTIL_QUALIFIER, t.identifier("exoticCell")), [])
    case "debugstr":
