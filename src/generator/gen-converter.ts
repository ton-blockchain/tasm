...
const CONVERT_UTIL_QUALIFIER = t.identifier("u")
...
const generate = (): string => {
  ...
  const convertUtilAst = t.importDeclaration(
    [t.importNamespaceSpecifier(CONVERT_UTIL_QUALIFIER)],
    t.stringLiteral("./convert-util"),
  )

  const stmts = [...instructionList(), ...fiftInstructionList()].flatMap(([name, instruction]) => {
  ...
  const file = t.file(
    t.program([importUtil, importConstructors, importAst, convertUtilAst, ...stmts, convertorFunc]),
  )
  t.addComment(
    file,
    "leading",
...
function generateBody(name: string, instruction: $.Opcode) {
  ...
  if (len === 0) {
    const lenCheck = t.expressionStatement(
      t.callExpression(t.memberExpression(CONVERT_UTIL_QUALIFIER, t.identifier("assertZeroArgs")), [
        t.identifier("instr"),
        t.identifier("loc"),
      ]),
    )
    return t.blockStatement([lenCheck, ...argsStatements])
  }

  if (len === 1) {
    const lenCheck = t.expressionStatement(
      t.callExpression(
        t.memberExpression(CONVERT_UTIL_QUALIFIER, t.identifier("assertSingleArgs")),
        [t.identifier("instr"), t.identifier("loc")],
      ),
    )
    return t.blockStatement([lenCheck, ...argsStatements])
  }
...
const generateSimpleArgs = (name: string, args: $.args): t.Statement[] => {
  ...
  if (args.length === 2) {
    ...
    if (isIntegerArg(arg) && isStackArg(arg1)) {
      return [parseArgs("twoIntegerArgs", false), returnTwoArgInstr(name)]
    }

    if (isStackArg(arg) && isStackArg(arg1)) {
      return [parseArgs("integerAndStackArgs", false), returnTwoArgInstr(name)]
    }
