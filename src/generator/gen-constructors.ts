...
const generateOpcode = (name: string, instruction: $.Opcode): t.Statement[] => {
    ...
    const lambda = t.arrowFunctionExpression(params, t.objectExpression(fields))
    lambda.returnType = t.tsTypeAnnotation(t.tsTypeReference(t.identifier(name)))

    const constructor = t.exportNamedDeclaration(
        t.variableDeclaration("const", [t.variableDeclarator(t.identifier(name), lambda)]),
    )
