...
function generatePrintFunction(
  ...
  printerParam.typeAnnotation = t.tsTypeAnnotation(
    t.tsTypeReference(t.tsQualifiedName(PRINTER_QUALIFIER, t.identifier("Printer"))),
  )
...
function writeToString(name: string) {
  const argMember = t.memberExpression(t.identifier("instr"), t.identifier(name))
  const toStringCall = t.callExpression(t.memberExpression(argMember, t.identifier("toString")), [])
  return t.expressionStatement(
    t.callExpression(t.memberExpression(t.identifier("p"), t.identifier("append")), [toStringCall]),
  )
}
...
const generateArg = (name: string, arg: $.arg): t.Statement[] => {
  switch (arg.$) {
    ...
    case "inlineCodeSlice":
      return [
        t.expressionStatement(
          t.callExpression(t.memberExpression(PRINTER_QUALIFIER, t.identifier("printCode")), [
            t.identifier("p"),
    ...
    case "slice":
      return [
        t.expressionStatement(
          t.callExpression(t.memberExpression(PRINTER_QUALIFIER, t.identifier("printSlice")), [
            t.identifier("p"),
            t.memberExpression(t.identifier("instr"), t.identifier(name)),
          ]),
        ),
...
const generateDictpush = (): t.Statement[] => {
  return [
    ...
    writeAppend(" "),
    t.expressionStatement(
      t.callExpression(t.memberExpression(PRINTER_QUALIFIER, t.identifier("printDictionary")), [
        t.identifier("p"),
        t.memberExpression(t.identifier("instr"), t.identifier("arg1")),
      ]),
    ),
  ]
}
...
function writeAppend(val: string) {
  return t.expressionStatement(
    t.callExpression(t.memberExpression(t.identifier("p"), t.identifier("append")), [
      t.stringLiteral(val),
    ]),
  )
}
