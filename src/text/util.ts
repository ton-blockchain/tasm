...
export const processInstructions = (ctx: Ctx, instructions: $ast.instructions): i.Instr[] => {
  return instructions.map(it => {
    ...
    try {
      if (it.$ === "ExplicitRef") {
        return i.PSEUDO_PUSHREF(i.util.code(processInstructions(ctx, it.code.instructions)), loc)
      }
      ...
      if (it.$ === "Exotic") {
        if (it.lib.$ === "DefaultExotic") {
          return i.PSEUDO_EXOTIC(DefaultExoticCell(parseDataLiteral(it.lib.data).asCell()))
        }
